...
export function CoffeeShopDetailPage({
  coffeeShop,
  isEditing,
  onFinishEditing,
}: {
  coffeeShop: Awaited<ReturnType<typeof getCoffeeShop>>
  isEditing: boolean
  ...
  >(null)
  const form = useFormState(() => createEditValues(coffeeShop))
  const { setValues: setFormData } = form

  useEffect(
    () => setFormData(createEditValues(coffeeShop)),
    [coffeeShop, setFormData],
  )
  ...
  const handleSave = async () => {
    if (!form.values.name.trim()) return toast.error('Café name is required')
    setIsSaving(true)
    try {
      await updateCoffeeShop({
        data: {
          ...coffeeShopUpdatePayload(coffeeShop.id, form.values),
        ...
        onToggleWantToVisit={async () => {
          ...
          try {
            ...
            toast.error(
              getErrorMessage(
                error,
                'Could not update your want-to-visit list',
              ),
            )
          } finally {
            setUpdatingList(null)
    ...
    <Card>
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-4">
        ...
        <Button size="sm" asChild>
          <Link to="/visits/new" search={{ coffeeShopId: coffeeShop.id }}>
            <Plus />
            Log a visit
          </Link>
        </Button>
