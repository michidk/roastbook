...
function OverviewMetrics({ stats }: Pick<OverviewData, 'stats'>) {
...
function RecentBrews({ recentShots }: Pick<OverviewData, 'recentShots'>) {
...
type RecentShot = OverviewData['recentShots'][number]

function RecentBrewRow({
  shot,
  now,
  formatDate,
  showRating,
  ...
  const brewedAt = new Date(shot.brewedAt)
  return (
    <Link
      to="/brews/$shotId"
    ...
    >
      ...
      <div className="flex shrink-0 self-stretch flex-col items-end justify-center gap-1">
        {isSameDay(brewedAt, now) ? null : (
          <time
            dateTime={brewedAt.toISOString()}
