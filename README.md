...
- **Machine research.** Pull documented factory espresso settings for your
  ...
  machines.
- **Brew recommendations.** Compare the current new-brew setup and parameters
  with up to 50 brews for the same bean, method, and exact gear setup, then
  suggest a small evidence-grounded adjustment for the next brew.
...
- Search beans, cafés, and gear directly; jump to any page; start a new brew,
  bean, or visit; and switch theme from the desktop command palette (`⌘K` or
  `Ctrl+K`).
