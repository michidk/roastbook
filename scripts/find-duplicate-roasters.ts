...
async function mergeGroups(duplicateGroups: RoasterWithBeanCount[][]) {
  for (const group of duplicateGroups) {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx
        .delete(schema.roasters)
        .where(inArray(schema.roasters.id, duplicateIds))
...
async function main() {
  ...
  if (duplicateGroups.length === 0) {
    console.log(
      'No duplicate roasters found (grouped by lowercase/trimmed/whitespace-collapsed name).',
    )
    await client.end()
    return
  }
  ...
  if (shouldMerge) {
    ...
    console.log(
      `Dry run only - no changes made. Re-run with ${MERGE_FLAG} to merge these groups.`,
    )
  }
